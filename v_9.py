...
# Loading Datas
###################################
df_ = pd.read_csv("inflation/csvData.csv")
...
df_[['dateReference_month', 'dateReference_year']] = df_['dateReference'].str.split('/', expand=True)
df_.drop("dateReference", axis=1, inplace=True)
df_.loc[df_["dateReference_month"] == "Oct","dateReference_month"] = "10"
df_[['dateReference_month', 'dateReference_year']] = df_[['dateReference_month', 'dateReference_year']].astype(int)
df_new[[i for i in df_.columns if i not in "country"]] = np.nan

for i,col in enumerate(df_new["CNation"]):
    for j,col_inf in enumerate(df_["country"]):
        if col == col_inf:
            df_new.iloc[i,-4:] = df_.iloc[j,1:]
            break
...
#Fifanın veriye eklenmesi
df_new=df_new[~df_new["Name"].isnull()]
df_new.reset_index(drop=True,inplace=True)

cat_cols_fifa,num_cols_fifa,cat_but_car_fifa = grab_col_names(df_Fifa)
delete_cols_fifa = ["sofifa_id","fifa_id","value_eur","wage_eur","age","height_cm","weight_kg","club_team_id",
               "club_jersey_number","nationality_id","nation_team_id","nation_jersey_number",
               "goalkeeping_diving","goalkeeping_handling","goalkeeping_kicking","goalkeeping_positioning",
               "goalkeeping_reflexes","goalkeeping_speed"]

num_cols_fifa = [i for i in num_cols_fifa if i not in delete_cols_fifa]
df_new[num_cols_fifa] = np.nan
...
for i,col_fm in enumerate(df_new["Name"]):
    col_fm = col_fm.replace("á", "a")
    col_fm =col_fm.replace("é", "e")
    col_fm = col_fm.replace("ń", "n")
    col_fm = col_fm.replace("ñ", "n")
    col_fm = col_fm.replace("ó", "o")
    lst_fm = str(col_fm).split(sep=" ")
    for j,col_fifa in enumerate(df_Fifa["long_name"]):
        col_fifa = col_fifa.replace("á", "a")
        col_fifa = col_fifa.replace("é", "e")
        col_fifa = col_fifa.replace("ń", "n")
        col_fifa = col_fifa.replace("ñ", "n")
        col_fifa = col_fifa.replace("ó", "o")
        lst_fifa = str(col_fifa).split(sep=" ")
        r = lst_fm
        r.sort()
        e =list(set(lst_fifa) & set(lst_fm)) #
        e.sort()
        if r == e:
            # print("FM: ", col_fm,"\nindex:",i,"\n","FIFA: ", col_fifa,"\n","İndex: ",j,"\n")
            #b = b + 1
            #index_lst_fifa.append(j)
            #index_lst_fm.append(i)
            df_new.loc[i,num_cols_fifa] = df_Fifa.loc[j,num_cols_fifa]
            break
#dfff = pd.DataFrame({"a":index_lst_fifa,"b":index_lst_fm},columns=["a","b"])
...
#df_new.drop(['Unnamed: 0','Link',"Release_Clause",'Unique_ID',"CName","CLink","CStatus","Name"], axis=1,inplace=True)
df_new.drop(["Unnamed: 0",'Link',"Release_Clause","Unique_ID_fm21",'Unique_ID',"Name","Release_Clause_fm21"], axis=1,inplace=True)
#Potential_fm21 çıkartılabilir
...
# Veri seti içerindeki string ama bizim için sayısal olan değerlere gerekli işlemleri
# yaparak dönüşümlerini gerçekleştirdik.
missing_values_table(df_new)
...
# # Sell_Value
# df_new = df_new.loc[df_new["Sell_Value"] != "Not for sale"].reset_index(drop=True)
# df_new["Sell_Value"] = df_new["Sell_Value"].str[2:].str.replace(",", "").astype(float)
# # Sell_Value_fm21
...
ax = sns.scatterplot(x =df_new['Potential'], y = df_new['Wages'], hue = df_new['Nation'])
plt.xlabel("Potential")
plt.ylabel("Wage EUR")
plt.title("Potential & wage", fontsize = 18)
plt.show()
...
df_new[df_new["Foot"] == "Left"]
############################
# Outlier
############################

# Filtreleme
df_new = df_new.loc[df_new["Wages"] < 40000]
df_new.shape
...
labelencoder = LabelEncoder()
encoder_df = df_new[["Nation", "CNation"]]
...
df_new = label_encoder(df_new, "CCity")
df_new = label_encoder(df_new, "Team")
df_new = label_encoder(df_new, "Team_fm21")
df_new = label_encoder(df_new, "CLeague")
...
df_new['Sell_Value'] = np.log1p(df_new["Sell_Value"].values)
...
np.expm1(y).mean()# 5254479.937480165
np.expm1(y).median()# 3305540.999999998
np.expm1(y).std()# 5418545.292900543
...
lgbm_model.fit(X_train,y_train,eval_metric="auc",eval_set=[(X_test,y_test)],early_stopping_rounds=10000)

cv_results = cross_validate(lgbm_model, X_train, y_train, cv=5, scoring=["neg_root_mean_squared_error", "neg_mean_absolute_error", "r2"])
...
df_new.corr()["Wages"].sort_values(ascending=False)
